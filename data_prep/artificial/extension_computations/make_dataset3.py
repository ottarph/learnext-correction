...
if __name__ == "__main__":
    ...
    with df.XDMFFile(working_dir+"/displacements3.xdmf") as infile:

        p = Path(working_dir+"/harmonic3.xdmf")
        if p.exists(): p.unlink()
        p = Path(working_dir+"/biharmonic3.xdmf")
        if p.exists(): p.unlink()
        outfile_harm = df.XDMFFile(working_dir+"/harmonic3.xdmf")
        outfile_biharm = df.XDMFFile(working_dir+"/biharmonic3.xdmf")
