...
if __name__ == "__main__":
    ...
    with df.XDMFFile(working_dir+"/displacements2.xdmf") as infile:

        p = Path(working_dir+"/harmonic2.xdmf")
        if p.exists(): p.unlink()
        p = Path(working_dir+"/biharmonic2.xdmf")
        if p.exists(): p.unlink()
        outfile_harm = df.XDMFFile(working_dir+"/harmonic2.xdmf")
        outfile_biharm = df.XDMFFile(working_dir+"/biharmonic2.xdmf")
