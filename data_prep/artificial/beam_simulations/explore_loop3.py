...
if __name__ == "__main__":
    ...
    p = Path(working_dir+"/displacements3.xdmf")
    if p.exists():
        p.unlink()
    p = Path(working_dir+"/displacements3.h5")
    if p.exists():
        p.unlink()
    displacement_file = df.XDMFFile(working_dir+"/displacements3.xdmf")
    ...
    for k, theta in enumerate(np.linspace(0, 2*np.pi, 101)):
        ...
        six_load = df.Expression(('0', 'abs(x[0] - 0.5) < 0.04? C: 0'), degree=1, C=-0.2E3*np.cos(theta+0.0*np.pi))
